...
import { demoAccountHintsEnabled } from "@/lib/demo-accounts";
  ...
  return (
    <MinimalAuthShell title="Sign in to your business account.">
      ...
      <p className="mb-4 text-sm text-[color:rgba(37,34,30,0.78)]">
        Partner sign-in starts from a fresh Clerk session on this page. Pick one of the five host audience demos below
        (coffee, cafe, studio, listening room, or bookshop), then continue to the business dashboard.
      </p>
      {!demoAccountHintsEnabled() && partnerGraderEmailHint ? (
        <div
        ...
        >
          ...
          <p className="mt-1">
            ...
            password in <code className="rounded bg-black/5 px-1 py-0.5 text-xs">docs/DEMO_SCRIPT.md</code>. If Clerk
            asks for an email code, use <span className="font-semibold">424242</span> for <code>+clerk_test</code>{" "}
            addresses.
          </p>
        </div>
      ) : null}
      {!demoAccountHintsEnabled() && partnerTesterEmailHint ? (
        <div
          className="mx-auto mb-4 max-w-md rounded-[1.25rem] border border-black/10 bg-white/75 px-4 py-3 text-sm text-[color:rgba(37,34,30,0.78)]"
        ...
        >
          ...
          <p className="mt-1">
            ...
            password in <code className="rounded bg-black/5 px-1 py-0.5 text-xs">docs/DEMO_SCRIPT.md</code>. Use this
            account when you want onboarding answers to persist without disturbing the grader profile.
          </p>
        </div>
      ) : null}
